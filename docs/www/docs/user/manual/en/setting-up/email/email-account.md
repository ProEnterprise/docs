...
You can manage multiple incoming and outgoing Email Accounts in Pro Enterprise ERP. There has to be at-least one default outgoing account and one default incoming account. If you are on the Pro Enterprise ERP cloud, the default outgoing email is set by us.
...
Pro Enterprise ERP will create templates for a bunch of email accounts by default. Not all of them are enabled. To enable them, you must set your account details.
...
Email Accounts allows you to set conditions according to the data of the incoming emails. The email will be imported to Pro Enterprise ERP only if the all conditions are true. For example if you want to import an email if the subject is "Some important email", you put doc.subject == "Some important email" in the conditions textbox. You can also set more complex conditions by combining them, as shown on the following screenshot.

<img class="screenshot" alt="Incoming EMail Conditions" src="/docs/assets/img/setup/email/email-account-incoming-conditions.png">

### How Pro Enterprise ERP handles replies

In Pro Enterprise ERP when you send an email to a contact like a customer, the sender will be the user who sent the email. In the **Reply-To** property, the Email Address will be of the default incoming account (like `replies@yourcompany.com`). Pro Enterprise ERP will automatically extract these emails from the incoming account and tag it to the relevant communication

### Notification for unreplied messages

If you would like Pro Enterprise ERP to notify you if an email is unreplied for a certain amount of time, then you can set **Notify if Unreplied**. Here you can set the number of minutes to wait before notifications are sent and whom the notifications must go to.
